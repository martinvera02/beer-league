...
export default function Navbar({ currentPage, setCurrentPage }) {
  ...
  const tabs = [
    ...
    { id: 'warlord',       emoji: '🏴‍☠️', label: 'warlord' },
  ...
  return (
    <>
        ...
        {toast && (
          <motion.div
            ...
            <div className="rounded-2xl px-4 py-3 flex items-center gap-3 shadow-2xl"
              ...
              <span className="text-2xl flex-shrink-0">{toast.type === 'powerup' ? '⚡' : '💸'}</span>
              ...
              <motion.button whileTap={{ scale: 0.9 }} onClick={dismissToast} className="text-white/60 text-lg flex-shrink-0">✕</motion.button>
            </div>
      ...
      {/* Navbar */}
      <nav className="fixed bottom-0 left-0 right-0 border-t px-1 pb-2 pt-1 z-50 transition-colors duration-300"
        style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        <div className="flex justify-around max-w-lg mx-auto">
          {tabs.map(tab => (
            <motion.button key={tab.id} onClick={() => handleTabClick(tab.id)}
              whileTap={{ scale: 0.85 }}
              className="relative flex flex-col items-center py-2 px-1 rounded-xl transition-colors"
              ...
              {currentPage === tab.id && (
                <motion.div layoutId="nav-indicator" className="absolute inset-0 rounded-xl"
                  style={{ backgroundColor: tab.id === 'clanwar' ? 'rgba(220,38,38,0.1)' : 'rgba(245,158,11,0.1)' }}
                  transition={{ type: 'spring', stiffness: 400, damping: 30 }} />
              )}
              <motion.span className="text-xl relative"
              ...
              <span className="text-xs mt-0.5 font-medium"
                style={{ color: currentPage === tab.id ? (tab.id === 'clanwar' ? '#ef4444' : '#f59e0b') : 'var(--text-hint)' }}>
                {tab.label}
              </span>
            </motion.button>
          ))}

          <motion.button onClick={() => setShowLogoutConfirm(true)} whileTap={{ scale: 0.85 }}

...
export default function Profile() {
  ...
  // Toque secreto en el avatar para abrir el panel admin
            ...
            <motion.div {...fadeIn} key="profile">
              <div className="rounded-2xl p-6 mb-4 text-center" style={{ backgroundColor: 'var(--bg-card)' }}>
                <div className="relative inline-block mb-3">
                  {profile?.avatar_url ? (
                    <img
                      ...
                      className="w-24 h-24 rounded-full object-cover border-4 border-amber-500 cursor-pointer"
                    />
                  ) : (
                    <div
                      onClick={handleAvatarTap}
                      className="w-24 h-24 rounded-full border-4 flex items-center justify-center text-4xl cursor-pointer"
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  {profile?.is_admin && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-amber-500 text-white font-bold">Admin</span>
                  )}
                </div>
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{user.email}</p>
                <p className="text-xs mt-1" style={{ color: 'var(--text-hint)' }}>
                  {uploadingAvatar ? 'Subiendo...' : profile?.is_admin ? 'Toca 5 veces el avatar para acceder al panel ⚙️' : 'Toca 📷 para cambiar tu foto'}
                </p>

                {/* Indicador visual de taps admin */}
                {profile?.is_admin && adminTapCount > 0 && (
                  <div className="flex justify-center gap-1 mt-2">
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className="w-1.5 h-1.5 rounded-full transition-colors"
                        style={{ backgroundColor: i <= adminTapCount ? '#f59e0b' : 'var(--bg-input)' }} />
                    ))}
                  </div>
                )}
              </div>
              ...
              <motion.div
                ...
                <div className="text-center mb-5">
                  ...
                  <p className="text-sm mt-2" style={{ color: 'var(--text-muted)' }}>
                    Se borrarán todos tus datos de forma permanente. Esta acción no se puede deshacer.
                  </p>
                </div>

...
async function detectAchievements(userId, stats, supabaseClient) {
  ...
  if (leagueIds.length > 0) {
    const top3Results = await Promise.all(
      leagueIds.map(lid =>
        supabaseClient.from('league_rankings')
          .select('user_id, total_points, total_drinks')
          .eq('league_id', lid)
          .order('total_points', { ascending: false })
          .limit(3)
      )
    )
    top3Results.forEach(({ data: top }) => {
      if (!top || top.length === 0) return
      if (top[0]?.user_id === userId) isTop1 = true
      if (top.some(r => r.user_id === userId)) isTop3 = true
      const byDrinks = [...top].sort((a, b) => b.total_drinks - a.total_drinks)
      if (byDrinks[0]?.user_id === userId) isMostActive = true
    })
  }

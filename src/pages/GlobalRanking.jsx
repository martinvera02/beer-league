...
function FreeRouletteWheel({ spinning, targetIndex, onSpinEnd }) {
  ...
  useEffect(() => {
    ...
    let startTime = null; const startRot = rotationRef.current
    const animate = (ts) => {
  ...
  const size = 260, cx = size/2, cy = size/2, r = size/2-8, SEGMENT_ANGLE = 360/FREE_SEGMENTS.length
  const polarToCartesian = (angle, radius) => { const rad = (angle-90)*Math.PI/180; return { x: cx+radius*Math.cos(rad), y: cy+radius*Math.sin(rad) } }
  const describeArc = (s, e) => { const st = polarToCartesian(s,r), en = polarToCartesian(e,r); return `M ${cx} ${cy} L ${st.x} ${st.y} A ${r} ${r} 0 ${e-s>180?1:0} 1 ${en.x} ${en.y} Z` }
  return (
    <div style={{ position:'relative', width:size, height:size, margin:'0 auto' }}>
      ...
      <div style={{ position:'absolute', inset:0, borderRadius:'50%', background:'linear-gradient(135deg,#fbbf24,#f59e0b)', padding:5, boxShadow:'0 0 30px rgba(245,158,11,0.4)' }}>
        <div style={{ width:'100%', height:'100%', borderRadius:'50%', overflow:'hidden' }}>
          <svg width={size-10} height={size-10} viewBox={`0 0 ${size} ${size}`} style={{ transform:`rotate(${displayRotation}deg)`, transformOrigin:'center' }}>
            {FREE_SEGMENTS.map((seg,i) => {
              const sa=i*SEGMENT_ANGLE, ea=(i+1)*SEGMENT_ANGLE, ma=sa+SEGMENT_ANGLE/2
              const tp=polarToCartesian(ma,r*0.62), ep=polarToCartesian(ma,r*0.84)
              return (<g key={i}><path d={describeArc(sa,ea)} fill={seg.color} stroke="rgba(0,0,0,0.12)" strokeWidth="1"/><text x={ep.x} y={ep.y} textAnchor="middle" dominantBaseline="middle" fontSize="13">{seg.emoji}</text><text x={tp.x} y={tp.y} textAnchor="middle" dominantBaseline="middle" fontSize="8.5" fontWeight="bold" fill={seg.textColor} transform={`rotate(${ma},${tp.x},${tp.y})`}>{seg.label}</text></g>)
            })}
...
function EuropeanRouletteWheel({ spinning, targetNumber, onSpinEnd }) {
  const [displayRotation, setDisplayRotation] = useState(0)
  const rotationRef = useRef(0); const animRef = useRef(null)
  useEffect(() => {
    if (!spinning || targetNumber === null) return
    cancelAnimationFrame(animRef.current)
    const targetIdx = WHEEL_ORDER.indexOf(targetNumber); if (targetIdx === -1) return
    const finalRotation = calcFinalRotation(rotationRef.current, targetIdx, 37, 6+Math.floor(Math.random()*4))
    const duration = 5000+Math.random()*1500; let startTime=null; const startRot=rotationRef.current
    const animate = (ts) => {
      if (!startTime) startTime=ts
      const progress = Math.min((ts-startTime)/duration,1)
  ...
  return (
    <div style={{ position:'relative', width:size, height:size, margin:'0 auto' }}>
      ...
      <div style={{ position:'absolute', inset:0, borderRadius:'50%', background:'linear-gradient(135deg,#92400e,#d97706,#92400e)', padding:6, boxShadow:'0 0 40px rgba(217,119,6,0.5)' }}>
        <div style={{ width:'100%', height:'100%', borderRadius:'50%', overflow:'hidden' }}>
          <svg width={size-12} height={size-12} viewBox={`0 0 ${size} ${size}`} style={{ transform:`rotate(${displayRotation}deg)`, transformOrigin:'center' }}>
            {WHEEL_ORDER.map((num,i) => {
              const sa=i*segAngle, ea=(i+1)*segAngle, ma=sa+segAngle/2
              const tp=polarToCartesian(ma,r*0.78), style=COLOR_STYLES[getNumberColor(num)]
              return (<g key={i}><path d={describeArc(sa,ea)} fill={style.bg} stroke="rgba(200,150,0,0.4)" strokeWidth="0.8"/><text x={tp.x} y={tp.y} textAnchor="middle" dominantBaseline="middle" fontSize="9" fontWeight="bold" fill={style.text} transform={`rotate(${ma},${tp.x},${tp.y})`}>{num}</text></g>)
            })}
...
function Casino() {
  ...
  const fetchBetData = async () => {
    ...
    setBetHistory(data || []); setLoadingBet(false)
  }
    ...
    <div className="max-w-md mx-auto px-4 pt-4 pb-6">
      ...
      <div className="flex rounded-xl p-1 mb-6" style={{ backgroundColor:'var(--bg-input)' }}>
        {[{ id:'free', label:'🎡 Ruleta Gratis', color:'#7c3aed' }, { id:'bet', label:'🎰 Ruleta Apuestas', color:'#b91c1c' }].map(t => (
          <button key={t.id} onClick={() => setCasinoTab(t.id)} className="relative flex-1 py-2 rounded-lg text-xs font-medium z-10" style={{ color: casinoTab===t.id ? '#fff' : 'var(--text-muted)' }}>
      ...
      {casinoTab === 'free' && (
        <>
          ...
          {loadingFree ? <div className="text-center py-10"><motion.div animate={{ rotate:360 }} transition={{ duration:1.5, repeat:Infinity, ease:'linear' }} className="text-4xl mb-2">🎡</motion.div></div> : (
            <>
              ...
              {spinHistory.length > 0 && (
                <div><p className="text-sm font-bold mb-3">Últimas tiradas</p>
        ...
        <>
          ...
          <AnimatePresence>
            {showBetResult && betResult && (
              <motion.div initial={{ opacity:0, scale:0.7 }} animate={{ opacity:1, scale:1 }} exit={{ opacity:0 }}
                ...
                <div className="flex justify-center mb-3"><div className="w-14 h-14 rounded-full flex items-center justify-center text-2xl font-black" style={COLOR_STYLES[betResult.color]}>{betResult.number}</div></div>
                <motion.p className="text-xl font-bold mb-1" animate={{ scale:[1,1.15,1] }} transition={{ duration:0.4 }} style={{ color: betResult.won ? '#10b981' : '#ef4444' }}>{betResult.won ? '¡GANASTE! 🎉' : '¡PERDISTE! 😬'}</motion.p>
                {betResult.won ? <><p className="text-2xl font-black text-emerald-400">+{betResult.payout}🪙</p><p className="text-xs mt-1" style={{ color:'var(--text-hint)' }}>x{betResult.multiplier} · Neto +{betResult.net}🪙</p></> : <p className="text-lg font-bold text-red-400">-{betResult.bet_amount||betAmount}🪙</p>}
          ...
          <div className="rounded-2xl p-4 mb-4" style={{ backgroundColor:'var(--bg-card)' }}>
            ...
            <div className="flex gap-2">
              {[50,100,250,500,1000].filter(v => v<=balance).map(v => (
                <motion.button key={v} whileTap={{ scale:0.9 }} onClick={() => setBetAmount(v)} className="flex-1 text-xs py-1.5 rounded-lg font-medium" style={{ backgroundColor: betAmount===v ? '#b91c1c' : 'var(--bg-input)', color: betAmount===v ? '#fff' : 'var(--text-muted)' }}>{v}</motion.button>
              ))}
            </div>
          </div>
          <div className="rounded-2xl p-4 mb-4" style={{ backgroundColor:'var(--bg-card)' }}>
            ...
            {betType === 'number' && (
              <div className="mt-3">
                <p className="text-xs mb-2" style={{ color:'var(--text-muted)' }}>Número: {betValue!==null ? <span className="font-black text-sm" style={COLOR_STYLES[getNumberColor(betValue)]}>{betValue}</span> : <span style={{ color:'var(--text-hint)' }}>ninguno</span>}</p>
                <div className="grid grid-cols-9 gap-1 max-h-44 overflow-y-auto">
                  ...
                  {Array.from({ length:36 }, (_,i) => i+1).map(n => (
                    <motion.button key={n} whileTap={{ scale:0.9 }} onClick={() => setBetValue(n)} className="aspect-square rounded-lg flex items-center justify-center text-xs font-bold"
          ...
          {betType && (
            <div className="rounded-xl p-3 mb-4" style={{ backgroundColor:'var(--bg-base)' }}>
              ...
              <div className="flex justify-between text-sm border-t pt-1 mt-1" style={{ borderColor:'var(--border)' }}>
                <span className="font-bold">Si ganas</span><span className="font-black text-emerald-400">{betAmount*parseInt(BET_TYPES.find(b => b.id===betType)?.payout?.replace('x','')||1)}🪙</span>
              </div>
            </div>
          )}
          <motion.button whileTap={{ scale:0.97 }} onClick={handleBetSpin}
            ...
            {betSpinning ? <div className="flex items-center justify-center gap-2"><motion.div className="w-5 h-5 rounded-full border-2 border-white border-t-transparent" animate={{ rotate:360 }} transition={{ duration:0.6, repeat:Infinity, ease:'linear' }}/>Girando...</div>
              : !betType ? '← Elige tipo' : betAmount>balance ? 'Saldo insuficiente' : betType==='number'&&betValue===null ? '← Elige número' : `🎰 Apostar ${betAmount}🪙`}
          ...
          {betHistory.length > 0 && (
            <div><p className="text-sm font-bold mb-3">Historial</p>
              <div className="space-y-2">
                {betHistory.map(bet => (
...
export default function GlobalRanking() {
  ...
  return (
    <div className="min-h-screen transition-colors duration-300" style={{ backgroundColor: 'var(--bg-base)', color: 'var(--text-primary)' }}>
      ...
      {/* Mercado directo sin sub-pestañas */}
      {tab === 'market' && <div className="pb-24"><Market /></div>}

      {tab === 'casino' && <div className="pb-24"><Casino /></div>}
    </div>
  )
}